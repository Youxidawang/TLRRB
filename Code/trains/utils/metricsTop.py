...
class MetricsTop():
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        eval_results = {
            ...
            "acc_2": f"{round(acc2, 4)}/{round(non_zeros_acc2, 4)}",
            "F1_score": f" {round(f_score, 4)}/{round(non_zeros_f1_score, 4)}",
            "Corr": round(corr, 4),
            "MAE": round(mae, 4)
        }
        return eval_results
